# Artifact Checks

Minimum checks before stage approval:

1. Required file exists for stage.
1. Stage `Status` field is set properly.
1. Success metric exists in PRD and learning review.
1. Acceptance criteria are testable.
1. Decision log entry exists for key changes.

Stage-specific required files:

1. discovery
- `reports/discovery-insights.md`
- `reports/discovery-brief.md`

1. prioritization
- `reports/prioritization-matrix.csv`
- `reports/roadmap-plan.md`
- `reports/prioritization-decision-log.md`

1. definition
- `reports/requirements-prd.md`
- `reports/execution-plan.md`

1. delivery_ready
- `reports/delivery-readiness.md`
- `reports/build-readiness.md` (`Status: Ready`)
- `reports/test-case-mapping.md`

1. launch_ready
- `reports/launch-readiness.md`
- `reports/release-checklist.md`

1. learning
- `reports/learning-review.md`
- `reports/iteration-backlog.md`
